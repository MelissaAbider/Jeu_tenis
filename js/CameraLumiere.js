function cameraLumiere(scene,camera){   // creation de la camera 
  ...
  var yPos=20;//document.forms["controle"].PosY.value;//*document.forms["controle"].zoom.value;
  var zPos=30;//document.forms["controle"].PosZ.value;//*document.forms["controle"].zoom.value;
...
//*************************************************************
 function lumiere(scene){
    ...
    lumPt1.position.set(20,10,20);
    lumPt1.intensity = 0.4;
    scene.add(lumPt1);

    let lumPt3 = new THREE.PointLight(0xffffff);
    lumPt3.castShadow = true;
    lumPt3.shadow.camera.far=2000;
    lumPt3.shadow.camera.near=0;
    lumPt3.position.set(-20,-10,20);
    lumPt3.intensity = 0.4;
    scene.add(lumPt3);
    ...
    lumPt2.position.set(-20,10,20);
    lumPt2.intensity = 0.4;
    scene.add(lumPt2);
...
//scene.add(lumDir)
 ...
 // scene.add(spotConique12);
  ...
  const dCyl=0.2;
  let LumiereDirectionnelle = new THREE.DirectionalLight(0xffffff);
  ...
  let LumiereDirectionnelle1 = new THREE.DirectionalLight(0xffffff);
  LumiereDirectionnelle1.position.set(1, 10, 1);
  LumiereDirectionnelle1.target.position.set(1,0,1);
  //LumiereDirectionnelle.position.set(1, 1, 2);
  //LumiereDirectionnelle.target.position.set(1,1,0);
  LumiereDirectionnelle1.target.updateMatrixWorld();// actualisation de "target"
  LumiereDirectionnelle1.castShadow = true;
  LumiereDirectionnelle1.shadow.camera.near = 0.2;
  LumiereDirectionnelle1.shadow.camera.far = 200;
  LumiereDirectionnelle1.shadow.camera.left = -dCyl;
  LumiereDirectionnelle1.shadow.camera.right = dCyl;
  LumiereDirectionnelle1.shadow.camera.top = dCyl;
  LumiereDirectionnelle1.shadow.camera.bottom = -dCyl;

  LumiereDirectionnelle1.intensity = 0.5;
  LumiereDirectionnelle1.shadow.mapSize.width = Math.pow(2,10);
  LumiereDirectionnelle1.shadow.mapSize.height = Math.pow(2,10);

  scene.add(LumiereDirectionnelle1);
